...
const handleCodeChange = async (code) => {
    ...
    if (code.length === 4) {
      const updatedTransfergabData = {
        ...requestBody,
        otp: code,
      };
  
      console.log('API transfert:', updatedTransfergabData);
  
      try {
        const response = await axios.post(`${baseURLtransfert}/transfers/initiate`, updatedTransfergabData);
  ...
  const onSummaryButtonClick = (transferID, ref) => {
    ...
    if (transferID && ref) {
      ...
      navigate('/congratsgab', { state: { transferid: transferID, reference: ref } });
    } else {
